...
int express_bulk_transfer(int ep, unsigned char *b, int l )
{
    ...
    libusb_fill_bulk_transfer( xfr, m_handle,ep, b, l,cb_xfr, NULL, USB_TIMEOUT );
...
{
    int res;
    libusb_transfer *xfr = express_alloc_transfer();
    libusb_fill_bulk_transfer( xfr, m_handle,ep, b, l,cb_i2c, NULL, USB_TIMEOUT );
    if((res=libusb_submit_transfer( xfr )) < 0 )
    {
        express_free_transfer( xfr );
...
void express_set_interp_and_filter( int interp, int filter )
{
    ...
    msg[0] = FPGA_ADD | I2C_WR;
    // MSB
    msg[1] = FPGA_INT_FIL_REG;
    msg[2] = (filter&0x03)|((interp<<2)&0x0C);
...
int express_send_buffer( void *b, int l )
{
    ...
    res = express_bulk_transfer(EP2OUT,(unsigned char *)b, l);
    return res;
}
//
//
...
//        loggerf("DATV-Express Hardware not found");
    ...
    // Load the FX2 firmware
    if((m_express_status = express_firmware_load( "datvexpress.ihx"))<0)
    {
//        loggerf("FX2 firmware file not found");
    ...
    if((m_express_status = express_fpga_load( "datvexpress.rbf"))<0)
    {
//        loggerf("FPGA firmware file not found");
        return EXP_RBF;
    }
    m_express_status = EXP_OK;
    // Wait for FPGA to start to run
    express_wait();
    //
